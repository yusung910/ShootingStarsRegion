...
string DBManager::SearchAccount(InMemStream& in)
{
	string sRet;
	...
	try
	{
		...
		sql += " WHERE USER_ID = ?";
		sql += " AND USER_PW = ?;";
		pstmt = db.con->prepareStatement(sql.c_str());
...
vector<map<string, string>> DBManager::SelectCharacterList(const string& user_seq)
{
...
bool DBManager::InsertCharacter(const PlayerVO& vo)
{
	bool bRet = false;
...
bool DBManager::DeleteCharacter(const PlayerVO& vo)
{
	bool bRet = false;
	try
	{
